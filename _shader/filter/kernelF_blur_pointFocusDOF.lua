...
{
    {
        ...
        name = "uniSetting",
        paramName = {
            'Process','Focus_X','Focus_Y','Focus_Radius',
            'Falloff','Max_Blur','Focus_Boost','Aspect_Ratio',
            '','','','',
            '','','','',
        },
        default = {
...
vec2  Focus_Point   = vec2( u_UserData0[0][1], u_UserData0[0][2] );
float Focus_Radius  = u_UserData0[0][3];
float Falloff       = u_UserData0[1][0];
float Max_Blur      = u_UserData0[1][1];
float Focus_Boost   = u_UserData0[1][2];
...
P_COLOR vec4 FragmentKernel( P_UV vec2 UV )
{
    ...
    vec3 finalRGB = mix( tex.rgb, clamp( boosted, 0.0, 1.0 ), clamp( Process, 0.0, 1.0 ) );
