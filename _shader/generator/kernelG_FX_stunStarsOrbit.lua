...
{
    {
        ...
        paramName = {
            ...
            'Color_B','Count','Aspect_Ratio','Wiggle_Amount',
            'Spread_Width','Orbit_Angle','Wiggle_Speed','',
        },
        default = {
            .5,.35,.16,.05,
            2,.02,1,.9,
...
float Wiggle_Amount = u_UserData0[2][3];
float Spread_Width  = u_UserData0[3][0];
float Orbit_Angle   = u_UserData0[3][1];
float Wiggle_Speed  = u_UserData0[3][2];
...
P_COLOR vec4 FragmentKernel( P_UV vec2 UV )
{
    ...
    for ( int i = 0; i < 30; i++ )
    {
        ...
        vec2 orbitScaled = Orbit_Radius * Spread_Width;
        vec2 pos0 = vec2( cos( t ) * orbitScaled.x, sin( t ) * orbitScaled.y - orbitScaled.y );
        // Orbit tilt + vertical wiggle so the ring can wobble instead of staying flat.
        float ca = cos( Orbit_Angle );
        float sa = sin( Orbit_Angle );
        vec2 pos = vec2( pos0.x * ca - pos0.y * sa, pos0.x * sa + pos0.y * ca );
        pos.y += sin( t * Wiggle_Speed + phase * 2.0 ) * Wiggle_Amount;
        vec2 d = rel - pos;
