...
float Progress        = u_UserData1[0][0];
...
P_COLOR vec4 FragmentKernel( P_UV vec2 UV )
{
    ...
    // Progress lifecycle: 0 = shrunk + faded out, 1 = expanded + faded in.
    float anim = clamp( Progress, 0.0, 1.0 );
    float animScale = max( anim, 0.001 );
    vec2 rel=(UV-center) / animScale;
    float dist=length(rel);
    float glow=exp(-dist / max(Glow_Size,0.001)) * Glow_Intensity;
    float angle=atan(rel.y, rel.x) / (2.0 * 3.14159265) + 0.5;
    // Seam-free burst variation: sample noise on a circle so angle 0/1
    // (atan discontinuity at 9 o'clock, -X axis) meets continuously.
    float burstAng = angle * 6.2831853;
    float radial = valueNoise(vec2(cos(burstAng)*2.0+5.0, sin(burstAng)*2.0+5.0));
    ...
    int count=int(max(Spike_Count, 1.0));
    for(int i=0;i<64;i++){
        if(i>=count) break;
    ...
    float alpha=smoothstep(Alpha_Thresh, Alpha_Thresh+Edge_Smooth, combined) * smoothstep( 0.0, 0.25, anim );
