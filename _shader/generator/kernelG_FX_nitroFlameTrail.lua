...
float Flame_Width    = u_UserData1[0][0];
float Flame_Height   = u_UserData1[0][1];
...
float nitro_plume( vec2 uv, vec2 nozzle, float seed )
{
    ...
    // Flame_Height stretches how long the plume stays wide before pinching off.
    float taper = pow( t, 0.75 / max( Flame_Height, 0.05 ) );
    float width = mix( Flame_Width, 0.0, taper );
