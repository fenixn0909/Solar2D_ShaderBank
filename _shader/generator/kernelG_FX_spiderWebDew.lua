...
P_COLOR vec4 FragmentKernel( P_UV vec2 UV )
{
    ...
    for ( int dewIndex = 0; dewIndex < DEW_MAX; dewIndex++ ) {
        ...
        float fi = float( dewIndex ) + Seed * 7.0;

        float dAngIndex = floor( hash1( fi * 3.3 + Seed ) * Spoke_Count );
        float dAngle = dAngIndex * spokeAngle;
        float dRadIndex = floor( hash1( fi * 7.7 + Seed * 1.7 ) * Ring_Count ) + 0.5;
        float dRad = dRadIndex * ringSpacing;
        vec2 dPos = vec2( cos( dAngle ), sin( dAngle ) ) * dRad;

        float twinkle = pow( 0.5 + 0.5 * sin( CoronaTotalTime * Twinkle_Speed + fi * 3.0 + Seed ), 4.0 );
